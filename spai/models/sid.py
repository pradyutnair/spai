...
class PatchBasedMFViT(nn.Module):
    def __init__(
        ...
        self.to_out = nn.Sequential(
            nn.Linear(attn_embed_dim, cls_vector_dim, bias=False), nn.Dropout(dropout)
        )
    ...
    def forward_arbitrary_resolution_batch(
        self, x: list[torch.Tensor], feature_extraction_batch_size: int
    ) -> torch.Tensor:
        ...
        for img, export_dir in zip(x, export_dirs):
            ...
            if export_image_patches:
                ...
                for i in range(0, patched.size(1)):
                    ...
                    features.append(
                        self.mfvit.forward_with_export(
                ...
                for i in range(0, patched.size(1), feature_extraction_batch_size):
                    features.append(
                        self.mfvit(patched[0, i : i + feature_extraction_batch_size])
                    )
            x = torch.cat(features, dim=0)  # SUM(L_i) x D
            del features

            # Attend to patches.
            x, attn = self.patches_attention(
                x.unsqueeze(0), return_attn=True
            )  # 1 x D, 1 x L_i
            ...
            if export_image_patches:
                for i in range(0, patched.size(1)):
                    export_file = (
                        patches_attn_dir / f"{attn_list[i]:.3f}_patch_{i}_.png"
                    )
                    Image.fromarray(
                        (
                            patched[:, i]
...
class MFViT(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(self.vit, vision_transformer.VisionTransformer) or isinstance(
            self.vit, backbones.DINOv2Backbone
        ):
    ...
    def forward_with_export(
        self, x: torch.Tensor, export_file: pathlib.Path
    ) -> torch.Tensor:
        """Forward pass of a batch of images.
        ...
        Image.fromarray(
            (x.detach().cpu().permute(0, 2, 3, 1).squeeze(dim=0).numpy() * 255).astype(
                np.uint8
            )
        ).save(export_file)
        Image.fromarray(
            (
                hi_freq.detach().cpu().permute(0, 2, 3, 1).squeeze(dim=0).numpy() * 255
            ).astype(np.uint8)
        ).save(f"{export_file.parent}/{export_file.stem}_hi_freq{export_file.suffix}")
        Image.fromarray(
            (
                low_freq.detach().cpu().permute(0, 2, 3, 1).squeeze(dim=0).numpy() * 255
            ).astype(np.uint8)
        ).save(f"{export_file.parent}/{export_file.stem}_low_freq{export_file.suffix}")
    ...
    def export_onnx(self, export_file: pathlib.Path) -> None:
        ...
        class ExportableMFVit(nn.Module):
            def __init__(self):
                super().__init__()
                if isinstance(
                    outer_instance.vit, vision_transformer.VisionTransformer
                    ...
                    raise TypeError(
                        f"Unsupported backbone type: {type(outer_instance.vit)}"
                    )
            ...
            def forward(self, x: torch.Tensor) -> torch.Tensor:
                ...
                with torch.no_grad():
                    x, low_freq, hi_freq = outer_instance._extract_features(
                        x, low_freq, hi_freq
                    )

                x = outer_instance.features_processor.exportable_forward(
                    x, low_freq, hi_freq
                )
        ...
        # torch._dynamo.mark_dynamic(x, 0)
        onnx_options: torch.onnx.ExportOptions = torch.onnx.ExportOptions(
            dynamic_shapes=True
        )
        onnx_program: torch.onnx.ONNXProgram = torch.onnx.dynamo_export(
            model, x, export_options=onnx_options
    ...
    def export_onnx_without_fft(self, export_file: pathlib.Path) -> None:
        ...
        class ExportableMFVit(nn.Module):
            ...
            def forward(
                self, x: torch.Tensor, x_low: torch.Tensor, x_high: torch.Tensor
            ) -> torch.Tensor:
                """Input size: B x C x H x W"""
                # with torch.no_grad():
                x, x_low, x_high = self.outer_instance._extract_features(
                    x, x_low, x_high
                )
                x = self.outer_instance.features_processor.exportable_forward(
                    x, x_low, x_high
                )
                return x
        ...
        x_low = torch.clamp(x_low, min=0.0, max=1.0).to(x.dtype)
        x_hi = torch.clamp(x_hi, min=0.0, max=1.0).to(x.dtype)
        # Normalize all components according to ImageNet.
        x = self.backbone_norm(x)
...
class FrequencyRestorationEstimator(nn.Module):
    ...
    def exportable_forward(
        ...
        sim_x_low_freq_mean: torch.Tensor = sim_x_low_freq.mean(dim=-1)  # B x N
        sim_x_low_freq_std: torch.Tensor = utils.exportable_std(
            sim_x_low_freq, dim=-1
        )  # B x N
        sim_x_hi_freq_mean: torch.Tensor = sim_x_hi_freq.mean(dim=-1)  # B x N
        sim_x_hi_freq_std: torch.Tensor = utils.exportable_std(
            sim_x_hi_freq, dim=-1
        )  # B x N
        sim_low_freq_hi_freq_mean: torch.Tensor = sim_low_freq_hi_freq.mean(
            dim=-1
...
class FeatureImportanceProjector(nn.Module):
    ...
    def __init__(
        ...
        self.proj1 = Projector(
            proj_layers, 2 * proj_dim, proj_dim, input_norm=False, dropout=dropout
        )
        self.proj2 = Projector(
            proj_layers, proj_dim, proj_dim, input_norm=False, dropout=dropout
        )
...
class DenseIntermediateFeaturesProcessor(nn.Module):
    ...
    def __init__(
        ...
        if patch_projection:
            if patch_projection_per_feature:
                ...
                self.feature_specific_patch_projection = nn.ModuleList(
                    patch_projection_modules
                )
            else:
                patch_proj_layers: list[nn.Module] = [nn.Dropout(dropout)]
                for i in range(proj_layers):
        ...
        for i in range(proj_layers):
            proj1_layers.extend(
                [
                    nn.Linear(
                        input_dim if i == 0 and not patch_projection else proj_dim,
                        proj_dim,
                    ),
                    nn.ReLU(),
                    nn.Dropout(dropout),
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.feature_specific_patch_projection is not None:
            projected: list[torch.Tensor] = []
            for i, patch_projection in enumerate(
                self.feature_specific_patch_projection
            ):
...
def build_cls_vit(config) -> ClassificationVisionTransformer:
    ...
    if config.MODEL.VIT.FEATURES_PROCESSOR == "rine":
        ...
        cls_vector_dim: int = config.MODEL.VIT.EMBED_DIM * len(
            config.MODEL.VIT.INTERMEDIATE_LAYERS
        )
        ...
        raise RuntimeError(
            f"Unsupported features processor: {config.MODEL.VIT.FEATURES_PROCESSOR}"
        )
...
def build_mf_vit(config) -> MFViT:
    ...
    if (
        config.MODEL.FRE.ORIGINAL_IMAGE_FEATURES_BRANCH
    ...
    if config.MODEL.RESOLUTION_MODE == "fixed":
        ...
        raise RuntimeError(
            f"Unsupported resolution mode: {config.MODEL.RESOLUTION_MODE}"
        )
