...
class SemanticFeatureEmbedding(nn.Module):
    ...
    Supports both CLIP ViT-B/32 and ConvNeXt-XXLarge backbones.
    """
    def __init__(self, model_name="vit", pretrained=None, 
                 device='cuda' if torch.cuda.is_available() else 'cpu', proj_dim=512):
        super().__init__()
        self.device = device
        self.model_type = model_name.lower()  # Store model type for forward pass
        
        # Load model based on type
        if "vit" in self.model_type or "clip" in self.model_type:
            # CLIP ViT-B/32 configuration
            self.model_name = "ViT-B-32"
            self.pretrained = "openai" if pretrained is None else pretrained
            ...
            # Default ConvNeXt-XXLarge configuration
            self.model_name = "convnext_xxlarge"
            self.pretrained = "/scratch-shared/dl2_all_data/open_clip_pytorch_model.bin" if pretrained is None else pretrained
            self.clip_model, _, self.preprocess = open_clip.create_model_and_transforms(
                model_name=self.model_name,
                pretrained=self.pretrained,
                device=device
        ...
        # Define normalization parameters for each model type
    ...
    def preprocess_image(self, image_tensor):
        ...
        params = self.normalization_params[self.model_name]
        
        preprocess = Compose([
            Resize((224, 224)),  # Both models use 224x224 input
...
def build_mf_vit(config) -> MFViT:
    ...
    if config.TRAIN.MODE == "contrastive":
        ...
        semantics_head = ClassificationHeadSemantics(
            ...
            mlp_ratio=config.MODEL.CLS_HEAD.MLP_RATIO,
            dropout=config.MODEL.SID_DROPOUT,
            semantic_dim=512 # 1024 for convnext, 512 for CLIP
        )
    else:
        raise RuntimeError(f"Unsupported train mode: {config.TRAIN.MODE}")
